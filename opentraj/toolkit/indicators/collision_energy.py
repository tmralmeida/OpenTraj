...
def DCA_MTX(x_4d):
    ...
    DOT_Dp_Dv = np.multiply(Dp[:, :, 0], Dv[:, :, 0]) + np.multiply(
        Dp[:, :, 1], Dv[:, :, 1]
    )
    Dv_sq = (
        np.multiply(Dv[:, :, 0], Dv[:, :, 0])
        + np.multiply(Dv[:, :, 1], Dv[:, :, 1])
        + 1e-6
    )
    ...
    DCA = np.stack(
        [Dp[:, :, 0] + TTCA * Dv[:, :, 0], Dp[:, :, 1] + TTCA * Dv[:, :, 1]], axis=2
    )
...
def run(datasets, output_dir):
    ...
    for name in all_names:
        ...
        allttc_pd = pd.DataFrame(
            data=np.zeros((len(avg_ttc), 2)), columns=["name", "ttc"]
        )
        ...
        alldca_pd = pd.DataFrame(
            data=np.zeros((len(avg_dca), 2)), columns=["name", "dca"]
        )
        ...
        collision_energy = pd.DataFrame(
            data=np.zeros((len(all_E), 2)), columns=["name", "collision_energy"]
        )
    ...
    E_d = normalize_samples_with_histogram(
        datasets_collision_energy, max_n_samples=500, n_bins=50
    )
