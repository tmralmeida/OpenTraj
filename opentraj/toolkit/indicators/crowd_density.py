...
from opentraj.toolkit.test.load_all import get_datasets
        ...
        if len(frame) > 1:
            ...
            for pj in range(len(dist)):
                dens_t_i = (
                    ...
                    * np.sum(
                        1
                        / ((a * np.array(distNN[-1])) ** 2)
                        * np.exp(
                            -np.divide(
                                (pair_dist[pj] ** 2),
                                (2 * (a * np.array(distNN[-1])) ** 2),
                            )
                        )
                    )
                )
...
def run(datasets, output_dir):
    ...
    for ds_name in all_names:
        ...
        g_density = pd.DataFrame(
            data=np.zeros((len(global_dens), 2)), columns=["ds_name", "global_density"]
        )
    ...
    # down-sample each group.
    gdens_d = normalize_samples_with_histogram(
        all_global_density,
        max_n_samples=800,
        n_bins=50,
        quantile_interval=[0.05, 0.98],
    )
    ldens_d = normalize_samples_with_histogram(
        all_local_density,
        max_n_samples=800,
        n_bins=50,
    ...
    #     quantile_interval=[0.05, 0.95],
    # )
    # BNgdens_d = normalize_samples_with_histogram(
    #     all_global_density[-2:],
    #     max_n_samples=800,
    #     n_bins=50,
    #     quantile_interval=[0.05, 0.98],
    # )
    # BNldens_d = normalize_samples_with_histogram(
    #     all_local_density[-2:],
    #     max_n_samples=800,
    #     n_bins=50,
    #     quantile_interval=[0.05, 0.95],
    ...
    sns.swarmplot(y="global_density", x="title", data=df_gdens, size=1, ax=axs[0])
    ...
    sns.swarmplot(y="local_density", x="title", data=df_ldens, size=1, ax=axs[1])
    axs[1].set_ylim([0, 6])
    axs[1].set_yticks([0, 2.0, 4.0, 6.0])
