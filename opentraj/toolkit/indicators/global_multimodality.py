...
def Gauss_K(x, y, h):
    N = len(x)
    return (
        math.exp(-np.linalg.norm(x - y) ** 2 / (2 * h**2))
        / (2 * math.pi * h**2) ** N
    )
...
def entropies_set(opentraj_root, datasets_name, M=30):
    ...
    for name in trajectories_set:
        trajlet_entropy_file = os.path.join(
            entropies_dir, name + str(M) + "-entropy.npy"
        )
        if os.path.exists(trajlet_entropy_file):
            ...
            entropy_values = get_entropies(
                opentraj_root, trajectories_set[name], name, M
            )
...
def analyze_trajlets(trajlets, ds_name, max_clusters=21, bandwidth=0.2, plot=False):
    ...
    for t in tqdm(times, ds_name):
        ...
        if interpolation is None:
            X, _, interpolation = get_sample_at_time(t, trajlets, plot=False)
        else:
            X, _, interpolation = get_sample_at_time(
                t, trajlets, interpolation=interpolation, plot=False
            )
...
def analyze_dataset_loop(arguments, opentraj_root):
    ...
    if os.path.exists(trajlet_global_file):
        ...
        times, num_clusters, entropy = analyze_trajlets(trajlets, ds_name, plot=False)
...
def run(trajlets, opentraj_root, output_dir):
    ...
    if args.execution == "normal":
        ...
        results = [analyze_dataset_loop(arg, opentraj_root) for arg in arguments]
    ...
    plt.savefig(
        os.path.join(output_dir, "global_multimodality.pdf"), bbox_inches="tight", pad_inches=0
    )
    plt.show()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Calculate global " "multimodality indicators."
    )
