...
def speed_var_of_trajs(trajlets_np: np.ndarray):
    ...
    speed_var_samples = np.max(speed_values[::1], axis=1) - np.min(
        speed_values[::1], axis=1
    )
...
def run(trajlets, output_dir):
    ...
    speed_mean_values = normalize_samples_with_histogram(
        speed_mean_values, max_n_samples=300, n_bins=50, quantile_interval=[0, 0.92]
    )
    speed_var_values = normalize_samples_with_histogram(
        speed_var_values, max_n_samples=300, n_bins=50, quantile_interval=[0, 0.92]
    )
    acc_mean_values = normalize_samples_with_histogram(
        acc_mean_values, max_n_samples=300, n_bins=50, quantile_interval=[0.02, 0.92]
    )
    acc_max_values = normalize_samples_with_histogram(
        acc_max_values, max_n_samples=300, n_bins=50, quantile_interval=[0.02, 0.92]
    )

    # put samples in a DataFrame (required for seaborn plots)
    df_speed_mean = pd.concat(
        [
            pd.DataFrame(
                {
    ...
    plt.savefig(
        os.path.join(output_dir, "motion_properties.pdf"), dpi=500, bbox_inches="tight"
    )
...
if __name__ == "__main__":
    from opentraj.toolkit.test.load_all import (
        all_dataset_names,
        get_datasets,
        get_trajlets,
    )
